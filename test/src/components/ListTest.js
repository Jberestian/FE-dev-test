...
class ListTest extends Component{
    ...
    constructor(){
        super();
        this.state = {
            data: [{"id":"17d5653e-ab63-44cb-b1f6-403d9d7c48a0","name":"Twitterlist","location":"Namur","currency":62675},
    ...
    render(){
        // let filterName = this.props.name(
        //     (name)=>{
        //         return name.indexOf(this.state.search);
        //     }
        // );
        const  filterText  = this.props.name;
        return(
            <div>
                <input
                type='text'
                ...
                <table>
                    <tbody>{this.state.data
                        .filter(name=>{
                            return name.name.toLowerCase().indexOf(filterText) >= -1
                        })
